...
For deeper regression coverage against an already-bound device (the smoke test does its own load+bind cycle; these reuse a device the DT overlay already bound), `tests/all.sh` runs four focused scripts in sequence:

- `tests/sysfs_stream.sh` polls all channels via `/sys/.../in_*_raw` ~300 times and checks per-axis spread. Catches sysfs path regressions (wedged chip, scale flips).
- `tests/buffered_stream.sh` streams 500 samples via hrtimer + IIO buffer, asserts zero `mag = (0,0,0)` / `(-1,-1,-1)` glitches and per-axis 5–95 % spread stays within noise. Direct regression test for the byte-swap fix and the aux-master NACK filter.
- `tests/consistency.sh` reads both paths and compares per-channel medians. Catches divergence between the decode paths (the byte-swap bug only showed up in the buffered path; this test would have caught it).
- `tests/bind_cycle.sh` does 5× unbind/rebind, watching `dmesg` for refcount / use-after-free WARNs. Regression test for the `indio_dev->trig` cleanup fix.

Shared helpers live in `tests/lib.sh` (configfs trigger create/teardown, device discovery, IIO state reset, defer-stack cleanup trap).
...
**Aux-master glitch filter.** The chip's I2C master NACKs the AK09916 on ~1 % of aux cycles. When that happens, `EXT_SLV_SENS_DATA_00..07` reads back as 0x00 (bus held low) or 0xFF (bus pulled up), and the NACK can fire at any byte boundary, so any contiguous tail of the three mag axes can come back as 0 / -1. `icm20948_trigger_handler` substitutes any axis reading exactly 0 or 0xFFFF with the last good value cached in `icm->last_mag_{x,y,z}` (`int16_t`, stored as big-endian as read from the chip). The sysfs path is single-axis-per-read and can't filter the same way, so it can still occasionally surface the glitch — tests tolerate a small rate there.
